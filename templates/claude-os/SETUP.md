...
```
.claude/agents/{{AGENT_PREFIX}}-*.md  (8개)
...
2. `.claude/agents/{{AGENT_PREFIX}}-*.md` 8개 파일명을 `{prefix}-*.md`로 rename.
...
- `.claude/agents/{{AGENT_PREFIX}}-*.md` 8개 — 자동 치환 후 실제 prefix로 파일명이 바뀌어 있어야 한다. 남아 있는 `{{...}}` placeholder가 있다면 setup.ps1 재실행 또는 수동 정리.
