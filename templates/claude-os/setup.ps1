...
.DESCRIPTION
    ...
    Renames .claude/agents/{{AGENT_PREFIX}}-*.md files to use the real agent prefix.
...
# ---------- Rename agent files ----------
# Template stores agent files as "{{AGENT_PREFIX}}-{role}.md" literally in the file name.
...
if (Test-Path -LiteralPath $agentDir) {
    ...
    foreach ($af in $agentFiles) {
        if ($af.Name -like '*{{AGENT_PREFIX}}*') {
            $newName = $af.Name -replace [regex]::Escape('{{AGENT_PREFIX}}'), $AgentPrefix
