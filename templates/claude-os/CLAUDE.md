...
`.claude/agents/{{AGENT_PREFIX}}-*.md` 프로파일을 Agent tool로 실행한다. 임시 역할을 만들지 않는다.

| 에이전트 | 용도 |
| --- | --- |
| `{{AGENT_PREFIX}}-phase-orchestrator` | phase 선택·이어받기·ledger 정리 |
| `{{AGENT_PREFIX}}-implementer` | 구현 (도메인 코드 변경) |
| `{{AGENT_PREFIX}}-ui-specialist` | UI/표면 작업 (UI 없는 프로젝트면 제거 가능) |
| `{{AGENT_PREFIX}}-data-steward` | 데이터·schema·설정 일관성 |
| `{{AGENT_PREFIX}}-qa-reviewer` | QA 검토 (read-only) |
| `{{AGENT_PREFIX}}-docs-keeper` | 문서 갱신 |
| `{{AGENT_PREFIX}}-performance-investigator` | 성능·quota·실행시간 |
| `{{AGENT_PREFIX}}-research-investigator` | Stage 0 Discovery Research / Stage 3 Tech Research |
...
> 이 섹션은 setup 직후 도메인에 맞게 직접 작성한다. 아래 3개는 도메인 무관 골격이며, 추가 CRITICAL 규칙을 도메인에 맞게 5~9개 추가한다.

- CRITICAL: 동작 중인 시스템의 전체 재작성보다 작은 helper·모듈·컴포넌트로 **점진 확장**을 우선한다.
- CRITICAL: **표시 로직과 실제 규칙이 같은 helper/data를 공유**한다. UI/리포팅 전용 보정 계산을 따로 만들지 않는다.
- CRITICAL: 단일 진입 파일(예: `main.gd`, `Code.gs`, `index.ts`)은 **통합 지점**이다. 새 규칙은 가능하면 작은 모듈로 분리한다.

<!-- 여기에 도메인 종속 CRITICAL 규칙을 추가한다. 예시(Apps Script):
...
> setup 후 도메인에 맞게 5개 내외로 직접 작성한다. 아래는 형식 예시.

- {빈도 높은 작업 1}: {변경 지점 1} → {변경 지점 2} → {검증 항목}.
- {빈도 높은 작업 2}: {변경 지점 1} → {변경 지점 2} → {검증 항목}.
- {빈도 높은 작업 3}: ...
- {빈도 높은 작업 4}: ...
- {빈도 높은 작업 5}: ...
...
> setup 후 도메인에 맞게 작성한다. 아래는 일반화 가능한 골격 두 가지.

### A. 표시 로직과 규칙 분리

```
// 금지 — UI/리포팅이 규칙과 다른 값을 계산해서 표시·규칙 drift
display.value = raw * 1.2   // 표시 보정값을 표시 레이어에서 만듦

// 권장 — 같은 helper를 표시와 규칙이 공유
display.value = StatHelper.format(raw)
```

### B. 통합 지점에 새 규칙 직접 추가

```
// 금지 — 단일 진입 파일에 새 시스템 규칙을 본문 누적
function tick() {
    // ... 기존 수백 줄 ...
    updateNewSystemA();
    updateNewSystemB();
...
<!-- 여기에 도메인별 anti-pattern 3~5개를 추가한다. -->
