...
- 자연어 보고 끝에 자신의 `## Output Schema` (`.claude/agents/tnb-*.md` 정의)에 맞는 ` ```yaml` 블록을 포함한다. 3단계 리뷰 문서 작성 시 파싱하여 이슈 테이블에 매핑한다.
- 최대 500단어. 추가 구현은 하지 않는다.
- 모든 출력은 한국어.

| 에이전트 | 리뷰 관점 |
|---|---|
| `tnb-phase-orchestrator` | PM: scope 과부하, milestone 현실성, 완료 기준 검증 가능성, 단계 의존성 |
| `tnb-implementer` | 개발자: 기술 타당성, 구현 복잡도, 통합 진입 파일 비대화 위험, 회귀 가능성 |
| `tnb-qa-reviewer` | QA: acceptance criteria 완전성, manual QA 부하, 자동화 차단 항목, 테스트 불가 시나리오 |
| `tnb-data-steward` | 데이터: schema 계약 일관성, 식별 키 누락, 데이터 fallback 규칙 준수 |
| `tnb-ui-specialist` | UI: 표면 소유권 명확성, 리빌드 트리거, fallback 처리, 기존 layout 파괴 위험 |

> UI 표면이 없는 프로젝트라면 5번째를 `tnb-performance-investigator` 또는 다른 도메인 특화 에이전트로 교체한다.
...
reviewer: "tnb-phase-orchestrator, tnb-implementer, tnb-qa-reviewer, tnb-data-steward, tnb-ui-specialist"
---
```
...
- 임시 역할 생성 금지 — 반드시 `.claude/agents/tnb-*.md` 프로파일 사용.
