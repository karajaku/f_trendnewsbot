...
> 역할: f_trendnewsbot 프로젝트의 단일 권위 개발 프로세스 정의 — 기획부터 완료까지 각 단계, 산출물, DoD, 추적 체인을 명시한다.
...
**대상 규모**: 대형 필수, 중형 선택
**담당**: `/research` 커맨드 (discovery 모드) → `tnb-research-investigator` 에이전트
...
- 위 DoD 6항 충족
- `tnb-research-investigator`의 `overall_verdict`가 `complete` 또는 `partial`
...
**대상 규모**: 대형 필수, 중형 선택
**담당**: `/research` 커맨드 (technical 모드) → `tnb-research-investigator` 에이전트
...
**대상 규모**: 소형 이상
**담당**: `/plan-phase` 커맨드 → `tnb-phase-orchestrator` 위임
...
| 작업 성격 | 담당 에이전트 |
|---|---|
| 도메인 로직 전담 | `tnb-implementer` |
| UI/표면 레이아웃 전담 | `tnb-ui-specialist` |
| data/schema/설정 전담 | `tnb-data-steward` |
| 도메인 로직 + 데이터 혼합 | `tnb-implementer` (data 항목 별도 확인) |
| 도메인 로직 + UI 표시 혼합 | `tnb-implementer` (UI 전용 계산 분리 금지 — CLAUDE.md 규칙) |
| 순수 UI 레이아웃 변경 | `tnb-ui-specialist` |
...
| step 번호 재정렬 / step 추가 | `tnb-phase-orchestrator` 재호출 또는 사용자 확인 |
...
| 역할 | 담당 | 내용 |
|---|---|---|
| 정적 검토 | `tnb-qa-reviewer` | acceptance criteria 항목별 코드 검토, 데이터 키 등록 확인, git diff --check, 데이터 파싱 검증 |
