...
| 시스템 | 영향 방향 | 비고 |
|---|---|---|
| Google Gemini API (2.0 Flash, ADR-004) | 외부 의존 | summarizer가 호출. 일일 토큰·호출 hard cap 필수. 무료 tier 영구 (15 RPM / 1500 RPD). |
...
| 외부 패키지 (google-genai, feedparser, requests, beautifulsoup4, pyyaml, python-dateutil) | 내부 의존 | pyproject.toml 동결 버전. `requests`가 텔레그램 API 호출 겸용. ADR-004 로 `anthropic` → `google-genai` swap. |
...
- 하루 `global.max_items`(기본 10)건 초과 큐레이션 — 인지 부하 증가, 5분 읽기 목표 깨짐. (2026-05-20: "카테고리당 11건 이상" → 글로벌 상한 기준으로 변경, AC-2.1 개정)
...
- Gemini API 호출은 V1 단일 호출(점수+요약 동시) 시작. V2에서 quality 모니터링 후 분리 검토 `(tech-research.md 결론 #2)` (ADR-004: LLM provider 가 Anthropic Claude Haiku → Gemini 2.0 Flash 로 swap 됐으나 단일 호출 구조는 동일.)
...
- **AC-2.1**: 카테고리 3개(`AI 트렌드` → `농산물·유통` → `팜보스 관심 키워드`) 순서 고정. 표면 A(텔레그램)·표면 B(Pages) 모두 동일 순서. 전체 발송 건수는 `config/filters.yml`의 `global.max_items`(기본 10)건을 **글로벌 상한**으로 하며, 요약 후 score(AC-2.10 회사 영향 점수) 내림차순 상위 N건만 발송한다. 카테고리별 최소 건수는 보장하지 않으며(score가 낮으면 컷), 컷 결과 0건인 카테고리는 "오늘 새 뉴스 없음"으로 표기한다. score 동점은 입력 순서를 유지한다. (2026-05-20 추가 — "카테고리당 5~10건" 폐기, phase 03)
...
- **AC-5.3** (ADR-004 갱신, 2026-05-19): Gemini API quota·hard cap 초과 (또는 Gemini API rate limit 429 / RESOURCE_EXHAUSTED) 시 `QuotaExceededError` → `main()`이 잡아서 운영자 텔레그램 chat(`OPS_ALERT_CHAT_ID`)에 alert 메시지 발송 후 종료. 직원 단톡방·Pages는 게시하지 않음.
...
| Digest body | Filtered articles + summarize.md prompt + 회사 컨텍스트 (3법인 사업 영역·산지) | summarizer/client.py — Gemini API 단일 호출(점수+요약+회사 영향+카테고리 핵심), JSON mode (response_schema), schema validation (ADR-004) | `Digest` (항목별 score·summary·company_impact + 카테고리 headline + HTML body + 텔레그램 인덱스 + TL;DR 자동 추출 메타) |
...
```json
{
  ...
  "meta": {
    ...
    "tokens_in": 6800,
    "tokens_out": 2400,
    "pages_url": "https://karajaku.github.io/f_trendnewsbot/digest/2026-05-20.html"
  }
}
```
...
| 이름 | 매체 | 용도 |
|---|---|---|
| `GEMINI_API_KEY` | GitHub Actions Secret | Gemini API 인증 (ADR-004 — 발급: https://aistudio.google.com/app/apikey) |
...
- 2026-05-20: 일일 발송 건수 **글로벌 상한** 도입 (phase 03-digest-item-cap — 운영자 피드백 "뉴스 45건은 너무 많아 주요 뉴스로 줄일 필요, 하루 10건이면 충분"). 직접 원인은 2026-05-20 키워드 운영 보강으로 후보 풀이 2배 가까이 증가한 것(`phases/_hotfix-log/2026-05-20-sources-keywords-expansion.md` 후속 위험 항목 예측대로). AC-2.1 개정 — "카테고리당 5~10건" 폐기, score 내림차순 글로벌 상위 N건(`config/filters.yml` `global.max_items` 기본 10) 으로 변경. §2 범위 외 항목도 글로벌 상한 기준으로 갱신. 구현: `summarizer/render.py`의 `_select_top_items` 가 요약(score) 산출 후 `build_digest` 안에서 1회 컷 → item_count·TL;DR·HTML·텔레그램 전 표면 일관.
