...
- **AC-2.5**: 본문(A·B)에 "why it matters" 같은 LLM 코멘트 라인 미포함(생성 금지).
...
- **AC-2.7**: 표면 A 헤더 이모지 `📰` 1개 + 카테고리 번호 `①②③`. 본문 항목 내부에 이모지 사용하지 않음. 표면 B는 추가로 HTML 인라인 스타일로 카테고리 헤더 강조(`<h2>`). (2026-05-19 사용자 결정)
- **AC-2.8** (ADR-003 신규): 표면 B(Pages HTML)는 `<meta name="robots" content="noindex,nofollow">` 헤더 필수. 같은 디렉토리에 `docs/digest/robots.txt`로 `User-agent: * / Disallow: /` 배치. 검색엔진 크롤링 차단.
...
- **AC-6.6** (ADR-003 신규): Pages HTML은 매일 `docs/digest/YYYY-MM-DD.html`로 commit. 같은 날짜 재실행 시 동일 파일 덮어쓰기 + commit message에 "(rerun)" 표기. push 권한은 `GITHUB_TOKEN` 기본값 사용.
...
| Digest body | Filtered articles + summarize.md prompt | summarizer/client.py — Claude API 단일 호출(점수+요약), Prompt caching | `Digest` (카테고리별 항목·메타·HTML body·텔레그램 인덱스 텍스트 포함) |
