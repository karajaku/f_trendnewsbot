...
### GitHub Pages 활성화 (1회 수동 설정, ADR-003 + 2026-05-19 boundary 갱신)

**전제**: master 의 `docs/canonical/`·`docs/features/`·`docs/_extracted/` 같은 회사 사내 문서를 Pages 에 노출시키지 않기 위해 **`gh-pages` 전용 branch 운영**.

운영자가 1회 셋업:

1. 로컬에서 `gh-pages` orphan branch 생성·push (PowerShell):
   ```powershell
   git checkout --orphan gh-pages
   git rm -rf .
   "# Digest archive`nGenerated by f_trendnewsbot. See https://github.com/$owner/f_trendnewsbot." | Out-File -FilePath README.md -Encoding utf8
   ...
   git checkout master
   ```
2. GitHub.com → Repo Settings → Pages
3. Source: `Deploy from a branch`
4. Branch: **`gh-pages`**, Folder: **`/ (root)`**
5. Save → 1~2분 후 `https://{owner}.github.io/f_trendnewsbot/` 이 활성 (현재는 README 만 노출)
6. 다음 cron 실행 시 dispatcher 가 `digest/YYYY-MM-DD.html` 을 `gh-pages` branch 에 push → 자동 배포
...
- **GitHub Pages 활성화 (gh-pages branch)**: 위 §GitHub Pages 활성화 가이드 6단계 (orphan branch 생성·push 후 Settings → Pages → Source `Deploy from a branch`, **`gh-pages`**, `/ (root)`)
